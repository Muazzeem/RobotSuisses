<template>
  <section class="page-hero">
    ...
    <div class="container">
      ...
      <div
        class="hero-subtext"
        v-html="getLocaleField(data, 'hero_subtitle', $i18n.locale)"
      ></div>

      <!-- Buttons -->
      <div class="hero-actions" v-if="data?.buttons">
        <button :class="index === 0 ? 'btn-outline' : 'btn-primary'" v-for="(button, index) in data?.buttons" :key="index">
          {{ getLocaleField(button, 'cta_button_text', $i18n.locale) }}
        </button>
      </div>
    </div>
  </section>
